...
const char* const udpGASListener PROGMEM = "G.A.S_Listener"; // 
    ...
    bool UDPRemoteDevice::udpPublish(const String & stringData){
        if(udpClient.connect(address, DEFAULT_UDP_PORT)){
 ...
 */
bool startUdpServer(){
    if(udp.listen(DEFAULT_UDP_PORT)) {
        if(udpPtrLog != NULL){
            udpPtrLog->print("UDP Listening on IP: ");
            udpPtrLog->print(WiFi.localIP());
            udpPtrLog->print(F(":"));
            udpPtrLog->println(DEFAULT_UDP_PORT);
        }
...
bool udpPublish(IPAddress address, const String & stringData){
    ...
    if (udp.sendTo(message, address, DEFAULT_UDP_PORT)){
        if(udpPtrLog != NULL)
...
void udploop(){
        if(udpNeedBroadcasScan && udpScanTimer < millis()){
            if (udp.broadcast(udpGASHello)) //udp.sendTo(message,addr, DEFAULT_UDP_PORT)
